...
public class DBHandler {
    ...
    public HashSet<FastFood> getFastFoodByIdOrder(int order_id) throws SQLException {
        ...
        return result;
                ...
                public HashSet<NL> getAllNl() throws SQLException {
                Statement sm = conn.createStatement();
                ResultSet rs = sm.executeQuery("SELECT * FROM NGUYEN_LIEU");
                HashSet<NL> result = new HashSet<>();
                while (rs.next()) {

                                String ten = rs.getString("TEN");
                        String donvi = rs.getString("DON_VI");
                        int soluong = rs.getInt("SO_LUONG_TRONG_KHO");
                        double gia_nl = rs.getDouble("GIA_NL");
                        // create a new nl object using the retrieved data
                                NL nguyenlieu = new NL( ten,donvi, soluong, gia_nl);
            ...
            public HashSet<Report> getDataFromFunction() throws SQLException {
                HashSet<Report> result =new HashSet<>();
                CallableStatement stmt = conn.prepareCall("{ ? = call SUMTHU_THANG() }");
                CallableStatement stmt1 = conn.prepareCall("{ ? = call SUMCHI_THANG() }");
                ...
                while (rs.next() && rs1.next()) {
                        ...
                        double total1=rs1.getDouble(2);
                        Report baocao = new Report(thang, total/1000000.0,total1/1000000.0);
                        result.add(baocao);
                    }
                rs.close();
